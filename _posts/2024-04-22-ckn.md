...
Now that everyone has a clear head and I have a clear head regarding the kernel trick, we are prepared to introduce the so-called "Convolutional Kernel Network", or CKN to his friends. In this section, we recall the mathematical insights and computational issues to address to obtain the CKN architecture.

### Insights and computational complexity
While the kernel trick is elegant and appealing from a mathematical perspective, one can wonder how to implement it in practice. Formally, we define an image as a mapping $$I_0 : \Omega_0 \to \mathbb{R}^{3}$$ that takes into account the $$3$$ color channels. For two patches $$\mathbf{x}$$ and $$\mathbf{x}'$$ extracted from $$I_0$$, their corresponding representations in $$\mathcal{H}$$ writes $$\varphi_1(\mathbf{x})$$ and $$\varphi_1(\mathbf{x}')$$ thanks to the kernel trick and we are done :party_popper: ! Wait a minute, nothing shocks you? Indeed, the attentive reader might have noticed that $$\mathcal{H}$$ can be an ***infinite*** dimensional space which makes the practical implementation intractable :dizzy_face:!
...
The figure summarizes all those operations that lead to a Convolutional Kernel Layer. 

{% include figure.liquid path="assets/img/blog_ckn/CKN.png" class="img-fluid rounded z-depth-0" zoomable=true %}

In the end, we obtain a "feature map" $$I_1 : \Omega_1 \to \mathbb{R}^{p_1}$$ that can be applied again. One can now build a **Convolutional Kernel Network** by simply stacking several Convolutional Kernel Layers and the output of the CKN is a *convolutional representation* of the input image.
