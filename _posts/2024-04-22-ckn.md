...
$$
\begin{equation*}
\min_{\mathbf{W} \in \mathbb{R}^{p_\ell \times |\Omega_\ell|}} \frac{1}{n} \sum_{i=1}^n \mathcal{L}( y_i, \langle \mathbf{W} , I_{\ell}^i \rangle )  + \frac{\lambda}{2} \| \mathbf{W} \|_{\text{F}}^2
\end{equation*}
$$

where $\lVert \cdot \rVert_{\text{F}}$ is the Frobenius norm that extends the Euclidean norm to matrices, and, with abuse of notation, the maps $I_{\ell}^i$ are seen as matrices in $\mathbb{R}^{p_\ell \times |\Omega_\ell|}$. Then, the supervised convolutional kernel network formulation consists of jointly minimizing with respect to $\mathbf{W}$ in $\mathbb{R}^{p_\ell \times |\Omega_\ell|}$ and with respect to the set of filters $\mathbf{Z}_1,\ldots,\mathbf{Z}_L$, whose columns are constrained to be on the Euclidean sphere.
...
<pre><code class="language-python">scipy.signal.correlate2d : 8.63763425219804 seconds
torch.nn.functional.conv2d : 0.16174200270324945 seconds
cupyx.scipy.signal.correlate2d : 0.000588882714509964 seconds</code></pre>
