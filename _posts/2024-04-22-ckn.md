...
While the kernel trick is appealing from a mathematical perspective, one can wonder whether it can be computed in practice. Formally, we deine an image as a mapping $$I_0 : \Omega_0 \to \mathbb{R}^{3}$$ to take into account the $$3$$ color channels. For two patches $$\mathbf{x}$$ and $$\mathbf{x}'$$ extracted from $$I_0$$, their corresponding representations in $$\mathcal{H}$$ writes $$\varphi_1(\mathbf{x})$$ and $$\varphi_1(\mathbf{x}')$$ thanks to the kernel trick. However, $$\mathcal{H}$$ can be an ***infinite*** dimensional space which makes the practical implementation intractable :dizzy_face: !

Fortunately, our friend Nyström comes to our rescue! The Nyström's method approximates the feature representations $$\varphi_1(\mathbf{x})$$ and $$\varphi_1(\mathbf{x}')$$ by their projection $$\psi_1(\mathbf{x})$$ and $$\psi_1(\mathbf{x}')$$ onto a ***finite*** dimensional subspace $$\mathcal{F}$$ (see the figure below).
