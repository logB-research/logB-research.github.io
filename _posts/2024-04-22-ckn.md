...
Now that everyone has a clear head regarding the kernel trick, we are prepared to introduce the so-called "Convolutional Kernel Network" or CKN. In this section, we first recall the main challenges to put theory into practice and then present the building blocks of the CKN along with snippets of their implementation.
