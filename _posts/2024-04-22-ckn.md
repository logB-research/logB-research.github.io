...
Now that everyone has a clear head regarding the kernel trick, we are prepared to introduce the so-called "Convolutional Kernel Network" or CKN. In this section, we first study how the kernel trick is used in practice to represent local image neighborhoods with the kernel trick. Then, we present the building blocks of a one-layer CKN along with their practical implementation and finally explain how to extend it to a multi-layer CKN.
