...
Little-known fact: [`torch.conv2D`](https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html) computes a [cross-correlation](https://en.wikipedia.org/wiki/Cross-correlation), not a [convolution](https://en.wikipedia.org/wiki/Convolution) (yeah, the name is confusing!). But what is the difference ? The discrete convolution operator $$\ast$$ between two functions $$g$$ and $$h$$ is defined as
