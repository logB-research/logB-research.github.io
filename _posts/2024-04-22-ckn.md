...
where $$(W_\ell)_\ell$$ represent the convolution operations, $$(\sigma_\ell)_\ell$$ are pointwise non-linear functions, (e.g., ReLU), and $$(\gamma_\ell)_\ell$$ represent the pooling operations (see <d-cite key="paulin2016convolutional"></d-cite>).
...
where $$(P_{\ell})_{\ell}$$ represent the patch extractions, $$(W_{\ell})_{\ell}$$ the convolution operations, $$(\sigma_{\ell})_{\ell}$$ the kernel operations (which allows us to learn non-linearity in the RKHS), and $$(\gamma_{\ell})_{\ell}$$ the pooling operations.

## Getting your hands dirty 🖥️
In this section, we discuss how to reimplement the key components of the CKN to better understand how things work.
...
### Stonage ML
To better understand how things work, we decided to reimplement the CKN architecture without using modern Deep Learning frameworks. It saves you the burden of reading hundreds of documentation pages, but also has its disadvantages notably regarding computational efficiency. We provide our open-source implementation from scratch [here](https://github.com/ozekri/CKN_from_Scratch).

#### Autodiff

$$
\begin{equation*}
\min_{\mathbf{W} \in \mathbb{R}^{p_\ell \times \lvert\Omega_\ell\rvert}} \frac{1}{n} \sum_{i=1}^n \mathcal{L}( y_i, \langle \mathbf{W} , I_{\ell}^i \rangle )  + \frac{\lambda}{2} \lVert \mathbf{W} \rVert_{\text{F}}^2
\end{equation*}
$$

where $$\lVert \cdot \rVert_{\text{F}}$$ is the Frobenius norm that extends the Euclidean norm to matrices, and, with abuse of notation, the maps $$I_{\ell}^i$$ are seen as matrices in $$\mathbb{R}^{p_\ell \times \lvert\Omega_\ell\rvert}$$. Then, the supervised convolutional kernel network formulation consists of jointly minimizing with respect to $$\mathbf{W} \in \mathbb{R}^{p_\ell \times \lvert\Omega_\ell\rvert}$$ and with respect to the set of filters $$\mathbf{Z}_1,\ldots,\mathbf{Z}_L$$, whose columns are constrained to be on the Euclidean sphere.
...
For implementation from scratch details, see this really simple [blog post](https://e-dorigatti.github.io/math/deep%20learning/2020/04/07/autodiff.html) from [Emilio Dorigatti](https://e-dorigatti.github.io/) website.
...
{% details Nvidia CUDA Implementation %}
{% highlight cpp %}
/*!
