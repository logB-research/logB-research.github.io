...
$$
\begin{equation*}
\min_{\mathbf{W} \in \mathbb{R}^{p_L \times \lvert\Omega_L\rvert}} \frac{1}{n} \sum_{i=1}^n \mathcal{L}( y_i, \langle \mathbf{W} , I_{L}^i \rangle )  + \frac{\lambda}{2} \lVert \mathbf{W} \rVert_{\text{F}}^2
\end{equation*}
$$
...
Opitimizing with respect to \mathbf{W} is straightforward with any gradient-based method. Opitimizing with respect to the $$Z_j, j \in \{1,\ldots, L\}$$ is a bit more tricky, as you have to examine the quantity $$\nabla_{\mathbf{Z}_{j}} \mathcal{L} (y_i, \langle \mathbf{W} , I_{L}^i \rangle)$$, for $$j \in \{1,\ldots, L\}$$ to compute the derivative with respect to the $$Z_j, j \in \{1,\ldots, L\}$$
