...
## Kernel Trick 🧙‍♂️
Before diving into the CKN itself, we briefly recall what the \emph{kernel trick} stands for. In many situations, one needs to measure the similarity between inputs. Such inputs would typically be mapped to a high-dimensional space and compared by computing their inner product. More formally, place yourself in your favorite high-dimensional feature space $$\mathcal{H}$$ and consider a mapping $$\varphi\colon \mathcal{X} \to \mathcal{H}$$. The similarity between two inputs $$\mathbf{x}, \mathbf{x}'$$ the inner product $$\langle \varphi(\mathbf{x}), \varphi(\mathbf{x}')\rangle_{\mathcal{H}}$$. The kernel trick enables to avoid working with high-dimensional vectors in $$\mathcal{H}$$ and directly evaluate such pairwise comparison by kernel evaluation, i.e.,
...
The kernel trick presents many advantages, one of whom is that in practice, one can simply choose a kernel $$K$$ such that the map $$\phi$$ fulfilling the equation above exists, without needing to derive $$\phi$$ in closed form. This is in particular the case for Mercer and positive definite (pd) kernels []. For the interested reader, more details can be found in <d-cite key="scholkopf2000kerneltrick"></d-cite>. In addition, the [slides](https://mva-kernel-methods.github.io/course-2023-2024/static_files/materials/slides.pdf) of [Julien Mairal](https://lear.inrialpes.fr/people/mairal/) and [Jean-Philippe Vert](https://jpvert.github.io/) contain all one needs to know about kernel methods. For the sake of self-consistency, we recall for the interested reader the definition of positive definite (pd) kernels along with an example of such kernel.
