...
Surprisingly, the converse is also true, i.e., any p.d. kernel can be expressed as an inner product. More formally, the Theorem of Reproducing Kernels (Aronzjerna, 1950) states that $$K$$ is a p.d. kernel on the set $$\mathcaL{x}$$ **if and only if** there exists a Hilbert Space $\mathcal{H}$$ and a mapping $$\varpsi \colon \mathcal{X} \to \mathcal{H}$$ such that
