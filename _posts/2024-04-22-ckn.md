...
One of the most prominent examples of kernels for which a map $$\phi$$ such that the above equation exists are positive definite (p.d.) kernels. Formally, a p.d. kernel is a function  
...
The original CKN paper <d-cite key="mairal2016endtoend"></d-cite> uses dot product kernels that are a specific type of p.d. kernels. A dot product kernel $$K \colon \mathcal{X}\times\mathcal{X} \to \mathbb{R}$$ verifies $$\forall \mathbf{x},\mathbf{x}' \in \mathcal{X},~ K(\mathbf{x},\mathbf{x}') = \kappa(\langle \mathbf{x},\mathbf{x}'\rangle)$$ for some $$\kappa \colon \mathbb{R}\to\mathbb{R}$$. Similarly, homogeneous dot product kernels are defined as $$\forall \mathbf{x},\mathbf{x}' \in \mathcal{X},~ K(\mathbf{x},\mathbf{x}') = \lVert \mathbf{x} \rVert\lVert \mathbf{x}'\rVert \kappa \left(\left\langle \frac{\mathbf{x}}{\lVert \mathbf{x} \rVert},\frac{\mathbf{x}'}{\lVert \mathbf{x}'\rVert}\right\rangle\right)$$. Under some regularity properties of $$\kappa$$ (see <d-cite key="mairal2016endtoend"></d-cite>), homogeneous dot product kernels can be used to represent image local neighborhoods in $$\mathcal{H}$$. A common example of dot product kernel is the *exponential* dot product kernel $$K_{\exp}(\mathbf{x},\mathbf{x}') = \text{e}^{\beta(\langle \mathbf{x},\mathbf{x}'\rangle -1)} = \text{e}^{-\frac{\beta}{2}\lVert \mathbf{x}-\mathbf{x}' \rVert_2^2}$$ for some $$\beta > 0$$. When $$\displaystyle\beta = \frac{1}{\sigma^2}$$, we recover the well-known Gaussian Kernel.
{% enddetails %}

One of the most direct and popular applications of the kernel trick is with Support Vector Machines (SVMs) where similarities are measured with a dot product in a high-dimensional feature space. Using the kernel trick makes the classification task easier and only requires kernel pairwise evaluation instead of explicitly manipulating high-dimensional vectors. We provide below a short animation that shows how the kernel trick can be used to map data initially contained in a 2D plane, to a larger space where they can be separated by a hyperplane.
