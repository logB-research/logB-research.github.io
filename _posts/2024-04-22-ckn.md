...
where $\|.\|_{\text{F}}$ is the Frobenius norm that extends the Euclidean norm to matrices, and, with an abuse of notation, the maps $I_{\ell}^i$ are seen as matrices in $\mathbb{R}^{p_\ell \times |\Omega_\ell|}$. Then, the supervised convolutional kernel network formulation consists of jointly minimizing with respect to $\mathbf{W}$ in $\mathbb{R}^{p_\ell \times |\Omega_\ell|}$ and with respect to the set of filters $\mathbf{Z}_1,\ldots,\mathbf{Z}_L$, whose columns are constrained to be on the Euclidean sphere.
