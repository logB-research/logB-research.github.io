...
The function $$\kappa(\langle \cdot, \cdot \rangle)$$ should be a smooth dot-product kernel on the sphere whose Taylor expansion has non-negative coefficients to ensure positive definiteness. A common example of appropriate dot-product kernel is the exponential kernel 

$$
\begin{equation*}
\kappa_{\exp}(\mathbf{x},\mathbf{x}') = \text{e}^{\beta(\langle \mathbf{x},\mathbf{x}'\rangle -1)} = \text{e}^{-\frac{\beta}{2}\lVert \mathbf{x}-\mathbf{x}' \rVert_2^2}
\end{equation*},
$$
