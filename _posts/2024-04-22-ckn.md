...
A direct and very popular application of the kernel trick is with Support Vector Machines (SVMs). THe input data are first embedded to a RKHS  allows us to map data points from an initial space to a more abstract RKHS. In this way, we can move into spaces in which the data is represented in a linearly separable way (by a hyperplane). Here's a short animation showing how the kernel trick can be used to map data initially contained in a 2D plane, to a larger space where they can be separated by a hyperplane.
...
In this section, we discuss how to reimplement the key components of the CKN from scratch.
