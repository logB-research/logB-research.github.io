...
- We extract patches $$\mathbf{x}$$ from the image $$I_0$$. {% details Code %}
    {% highlight python %}
...
where $$M_1$$ is the "feature map" after the second point operation. That is why, we can interpret the pooling operation as a "convolution" operation. {% details Code %}
    {% highlight python %}
