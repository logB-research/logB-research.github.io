...
Little-known fact: [`torch.conv2D`](https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html) computes a [cross-correlation](https://en.wikipedia.org/wiki/Cross-correlation), not a [convolution](https://en.wikipedia.org/wiki/Convolution) (yeah, the name is confusing!). But what is the difference ? The convolution operator $$\ast$$ between two functions $$f$$ and $$g$$ is defined as
...
whereas the cross-correlation operator $$\ast$$ is defined as

$$
\begin{equation*}
(f \circledast g)(t):=\int_{-\infty}^{\infty} \overline{f(\tau)} g(t+\tau) d \tau
\end{equation*}
$$

It's subtle, but in the case of images, cross-correlation requires one less "image flipping" than convolution. Given the number of convolutions we're going to calculate, if we can spare ourselves an "image flip" each time, we'll do it! What's more, as the filter parameters are learnable, it doesn't matter if we choose cross-correlation rather than convolution.
