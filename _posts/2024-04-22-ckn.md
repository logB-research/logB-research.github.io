...
Now that everyone has a clear head regarding the kernel trick, we are prepared to introduce the so-called "Convolutional Kernel Network" or CKN. In this section, we first study how the kernel trick can be used in practice to represent local image neighborhoods. Then, we review the three principles behind the Convolutional Kernel layer implementation and finally explain how to stack them to obtain a multi-layer CKN.

### From theory to practice: Kernel trick with Nystrom approximation
While the kernel trick is elegant and appealing from a mathematical perspective, one can wonder how to implement it in practice. Formally, we define an image as a mapping $$I_0 \colon \Omega_0 \to \mathbb{R}^{3}$$ that takes into account the $$3$$ color channels. For two patches $$\mathbf{x}$$ and $$\mathbf{x}'$$ extracted from $$I_0$$, their corresponding representations in $$\mathcal{H}$$ writes $$\varphi_1(\mathbf{x})$$ and $$\varphi_1(\mathbf{x}')$$ thanks to the kernel trick and we are done :party_popper: ! Wait a minute, nothing shocks you? Indeed, the attentive reader might have noticed that $$\mathcal{H}$$ can be an ***infinite*** dimensional space which makes the practical implementation intractable :dizzy_face:!
...
3. Apply pooling operations. Note that Gaussian linear pooling is defined as
...
On the one hand, A CNN of $$L$$ layer can be represented by its output $$f_{\text{CNN}}(\mathbf{x})$$, if $$\mathbf{x}$$ is the input, as \colon
