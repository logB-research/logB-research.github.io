...
A lot of these kernels exist, see <d-cite key="mairal2016endtoend"></d-cite> for details. These include the *exponential* dot product kernel $$K_{\exp}(\mathbf{x},\mathbf{x}') = \text{e}^{\beta(\langle \mathbf{x},\mathbf{x}'\rangle -1)} = \text{e}^{-\frac{\beta}{2}\lVert \mathbf{x}-\mathbf{x}' \rVert_2^2}$$, where $$\beta > 0$$. When $$\displaystyle\beta = \frac{1}{\sigma^2}$$, we recover the well-known Gaussian Kernel. By the way, the arguments of the chosen kernels can be learned during model training!
{% enddetails %}

One of the most direct and popular applications of the kernel trick is with Support Vector Machines (SVMs) where dot product in high-dimensional feature spaces can be computed using simple functions defined on pairs of inputs. In this way, input data that are not easily separable in the original space can be made linearly separable by mapping them to an arbitrary Hilbert space with the kernel trick. We provide below a short animation that shows how the kernel trick can be used to map data initially contained in a 2D plane, to a larger space where they can be separated by a hyperplane. The kernel here used is the Radial Basis Function (RBF) or Gaussian kernel $$K_{\exp}(\mathbf{x},\mathbf{x}') = \text{e}^{-\frac{\lVert \mathbf{x}-\mathbf{x}' \rVert^2}{2\sigma^2}}$$.
